...
def listed_products(id):
    ...
    def pie_plot_items_listed(id):
        ...
        return pie_vendor_items
    
    b_fig.append(bar_plot_items_listed(id))
    p_fig.append(pie_plot_items_listed(id))
    return b_fig,p_fig
...
def pie_plot(labels,values,month):
    layout=go.Layout(title=f'Items Listed vs Sold Price for {month}',
                     width=500,
                    height=400)
    pie_vendor_items = go.Figure(data=[go.Pie(labels=labels, values=values, pull=[0, 0.1])],layout=layout)
    pie_vendor_items.update_layout(title={'y':0.9,
                                         'x':0.5,
                                         'xanchor': 'center',
                                         'yanchor': 'top'})                               
...
def monthly_sales(ven_id):
    ...
    for month in range(len(available_month)):
        ...
        b_fig.append(bar_plot(vendor_sold_items[month][0],vendor_sold_items[month][1],mon_name))
        p_fig.append(pie_plot(vendor_sold_items[month][0],vendor_sold_items[month][1],mon_name))    
